...
# CONFIG - edit these
# ============================================================
TOKEN = os.environ.get("BOT_TOKEN", "8656548047:AAGS_dZfB4vlRrocZeYMC7v0n7P2kgm6fuQ")  # @BotFather se token
...
def save_data(data=None):
    ...
    try:
        ...
        # agar DATA_FILE (/data/data.json) writable nahi (volume missing),
        # toh project folder mein default file use karo
        try:
            with open(_DEFAULT_DATA, "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
        except Exception as e:
...
def send_photo_url(chat_id, image_urls, caption=None, kb=None, parse_mode="HTML"):
    """QR image download karke multipart upload - 100% reliable.
    image_urls: ek URL ya list of URLs - pehla jo kaam kare wahi bhejo."""
...
# ============================================================
def upi_amount(n):
    """Amount UPI/API ke liye sahi format - 25.0 -> '25', 25.5 -> '25.5'.
    '25.0' float format se gateway payment match NAHI karta - yehi fix."""
    try:
        n = float(n)
    except Exception:
        return str(n)
...
def status_label(o):
    return {"pending": "⏳ Waiting", "delivered": "✅ Delivered", "rejected": "❌ Rejected", "payment_failed": "❌ Payment Failed", "timeout": "⏰ Expired"}.get(o["status"], o["status"])
