...
STATE_TTL_SEC = 120  # reuse the state token for 2 minutes (fewer requests = less rate-limit risk)

def get_server_state() -> tuple[str | None, str | None]:
    """Returns (state, error). Uses a short cache and 3 retries with backoff."""
    now = time.time()
    if _STATE_CACHE["state"] and now - _STATE_CACHE["ts"] < STATE_TTL_SEC:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if m:
                    ...
                    return st, None
                last_err = "HTTP 200 but no state token found (page format may have changed)"
            else:
                last_err = f"HTTP {res.status_code} from StockGro server"
        except Exception as e:
            last_err = f"{type(e).__name__}: {str(e)[:150]}"
        log.warning("state attempt %d failed: %s", attempt + 1, last_err)
...
def check_number_registered(phone: str):
    ...
    state, err = get_server_state()
    if not state:
        return False, None, {"error": f"StockGro server issue: {err}"}
